...
- `eval_discover` now reports the SDK's seven evaluator categories separately
  instead of combining compliance and multimodal and leaving consistency as
  `other`; its return-contract documentation now lists all six top-level keys.
- `eval_audit_pack` now preserves API-error and rendering provenance
  (`api_error_rate`, modality, raster DPI, and pdfium build) when rebuilding a
  current pdfhell report instead of silently resetting those fields.

### Changed

- `eval_tool_call_accuracy` keeps its original single-call mode and adds a
  trace mode that directly accepts `expected_tool_calls` plus the canonical
  `agent_trace` returned by `eval_ingest_trace`. This makes the documented
  ingest → score MCP workflow executable without dropping into Python.
